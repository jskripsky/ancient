...
bool JahresTabelle( void )
{
    ...
    char DPortName[2][13] = { "LPT1", "Printer.$$$" };
	...
	// Start Krankheitscodes Ausgabe:
	// ==============================

	DosWriteFile( Printer, 30, (void _far *)"Abwesenheiten Krankheitscodes " );

	BinToAscii( (DosYear() % 100) - 1, 2, (char _far *)tempstring, LEFT, 2 );
	DosWriteFile( Printer, 2, (void _far *)tempstring );
	...
	for( tagescounter = 0; tagescounter < 31; tagescounter++ )
		...
		if( (KMonatsSpalte[monatscounter].AnzahlTage - 1 < monatscounter) || (KMonatsSpalte[monatscounter].Tag[tagescounter].AbwDauer == 0) )
		    {
		    DosWriteFile( Printer, SPALTENLAENGE, (void _far *)string );
		    continue;
		    }

		BinToAscii( KMonatsSpalte[monatscounter].Tag[tagescounter].AbwDauer / 60, 2, (char _far *)&(string[0]), RIGHT, 1 );
		string[2] = '.';
		BinToAscii( KMonatsSpalte[monatscounter].Tag[tagescounter].AbwDauer % 60, 2, (char _far *)&(string[3]), RIGHT, 2 );

		KMonatsSpalte[monatscounter].Tag[tagescounter].AbwProzent = (byte)Divide( 100 * KMonatsSpalte[monatscounter].Tag[tagescounter].AbwDauer, ATag );
		BinToAscii( KMonatsSpalte[monatscounter].Tag[tagescounter].AbwProzent, 3, (char _far *)&(string[6]), RIGHT, 1 );
		string[9] = '%';
	...
	for( monatscounter= 0; monatscounter < 12; monatscounter++ )
	    {
	    monatstotal = 0;
	    for( tagescounter = 0; tagescounter < KMonatsSpalte[monatscounter].AnzahlTage; tagescounter++ )
		monatstotal += KMonatsSpalte[monatscounter].Tag[tagescounter].AbwDauer;
	    ...
	    monatstotalprozent = (byte)Divide( (dword)(100 * monatstotal), (word)(ATag * KMonatsSpalte[monatscounter].AnzahlTage) );
...
bool Monat( void )
	...
	for( subcounter = 0; subcounter < Endtag; subcounter++ )
	    ...
	    //	ScrWriteStatus( "FEHLER!" );
	    //	ScrWritePossibleKeys( "<Enter>" );
	    //	ScrWriteInstructions( "Unkontrollierte Daten gefunden!", "<Enter> - Monatserfassung abbrechen." );
	    //
	    //	while( BiosGetKey() != KBENTER );
	    //
	    //	return( FALSE );
